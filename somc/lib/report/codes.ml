(* File generated by gen_codes_ml.py on Sun Mar  5 20:30:29 2023. *)
...
let error_name_from_code = function
  ...
  | 305 -> Some ("type", "module has no symbol")
  ...
  | 309 -> Some ("type", "other type error")
...
let explanation_from_code : int -> string option = function
  ...
  | 305 -> Some "This error is raised when trying to import- or use a symbol that does not exist from a module or file."
  | 306 -> Some "This error is raised when it is attempted to import- or use a private symbol from a module. Note that symbols starting with an underscore are implicitly private to their module."
  | 307 -> Some "This error is raised when it is attempted to import a symbol from a value/symbol that isn't a module."
  | 308 -> Some "This error indicates that the type of a value could not be inferred. Usually this can be solved with type annotations."
  | 309 -> Some "This error indicates a miscellaneous type error."
