...
(* let make_lazy = function
  | Lexpr_lambda _ as e -> e
  | e -> Lexpr_lazy e *)

(* lowering stuff *)

let lower_literal l =
  Latom_const (Matching.lower_literal l)
...
and lower_expr env expr =
  match expr.item with
    ...
    | Texp_switch cases ->
      let r = Env.fresh () in
      let scrut = Lexpr_atom (local r) in
      let cases' = List.map (fun (p, e) ->
        let env = bind_patt_vars env p in
        env, p, lower_expr env e) cases
      in
      let match' = Matching.lower_cases env scrut cases' in
      Lexpr_lambda ([r], match')
    ...
    | Texp_apply ({ item = Texp_primitive p; _ }, args)
      ...
      let prim_app = Lexpr_call (Latom_prim p, prim_args) in
      if extra_args = [] then prim_app
      else Lexpr_apply (prim_app, extra_args)
    ...
    | Texp_tuple es ->
      ...
      wrap_exprs_in_vars es' (fun rs -> Lexpr_tuple rs)
    ...
    | Texp_construct (ident, args) ->
      ...
      in
      let args' = List.map (lower_expr env) args in
      let expr args'' = Lexpr_object (tag, args'') in
      wrap_exprs_in_vars args' expr

    | Texp_literal l -> Lexpr_atom (lower_literal l)
    | Texp_ident i -> Lexpr_atom (Latom_var (Env.find env i.item))      
    ...
    | Texp_primitive p ->
      ...
      let call = Lexpr_call (
        Latom_prim p,
        List.map (fun a -> Lexpr_atom (local a)) args)
      in
      Lexpr_lambda (args, call)
...
let lower_toplevel env (tl : toplevel node) =
  match tl.item with
    | Ttl_value_def vdef ->
      ...
      let stmt = Lstmt_definition (var, expr) in
      ...
      let vars = match tdef.td_type.item with
        | TVariant rows ->
          ...
          let f (vars, tag) (ident, _) =
            ...
            Env.add ident (Lvar_tag tag) vars, tag + 1
          in
    ...
    | Ttl_extern_def edef ->
      ...
      let stmt = Lstmt_external (var, edef.ed_native_name.item, arity) in
      vars, Some stmt
