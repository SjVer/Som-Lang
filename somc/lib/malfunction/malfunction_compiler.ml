...
let rec reorder = function
  ...
  | Mnumop1(op, ty, t) ->
    reorder_sub `Pure (fun ev -> Mnumop1(op, ty, ev t))
  ...
  | Mconvert(src, dst, t) -> 
    reorder_sub `Pure (fun ev -> Mconvert(src, dst, ev t))
  ...
  | Mblock (n, ts) ->
    reorder_sub `Pure (fun ev -> Mblock(n, lrmap ev ts))

  | Mfield (n, t) ->
    reorder_sub `Impure (fun ev -> Mfield (n, ev t))

  | Mlazy e -> `Pure, Mlazy (snd (reorder e))

  | Mforce e ->
    reorder_sub `Impure (fun ev -> Mforce (ev e))
...
let lookup v =
  ...
  let (path, descr) =
    try Env.lookup_value ~loc:Location.none v !env
    with Not_found ->
...
let add_external name nat_name =
  let prim_descr = Primitive.simple ~name:nat_name ~arity:0 ~alloc:false in
  let open Types in
  let make desc =
    {
      desc = desc;
  ...
  (* let constr name = 
    ...
    let desc = Tconstr (Path.Pident ident, [], ref Mnil) in
    make desc
  in *)
  let descr =
    {
      val_type = make Tnil;
      val_kind = Val_prim prim_descr;
      val_loc = loc_none;
      val_attributes = [];
    }
  in
  env := Env.add_value name descr !env
...
let global_to_lambda = function
  ...
  | Glob_prim p ->
    ...
    let rec make_params n =
      if n <= 0 then []
      else fresh "prim" :: make_params (n-1)
    in
...
let rec to_lambda = function
  | Mvar v -> Lvar v
  | Mlambda (params, e) -> lfunction params (to_lambda e)
  | Mapply (fn, args) ->
    let ap_func fn = lapply fn (List.map to_lambda args) in
    begin match fn with
      | Mglobal v -> begin
        match lookup v with
          | Glob_prim p -> lprim (Pccall p) (List.map to_lambda args)
          | g -> ap_func (global_to_lambda g)
        end
      | fn -> ap_func (to_lambda fn)
    end
  | Mlet (bindings, body) ->
    bindings_to_lambda bindings (to_lambda body)
  | Mnum (`Int n) -> Lconst (Const_base (Const_int n))
  | Mnum (`Int32 n) -> Lconst (Const_base (Const_int32 n))
  | Mnum (`Int64 n) -> Lconst (Const_base (Const_int64 n))
  | Mnum (`Bigint n) -> begin
    match Z.to_int n with
      ...
      | exception Z.Overflow ->
        builtin ["Z"; "of_string"] [Lconst (Const_immstring (Z.to_string n))]
    end
  | Mnum (`Float64 f) -> Lconst (Const_base (Const_float (string_of_float f)))
  | Mstring s -> Lconst (Const_immstring s)
  | Mglobal v -> global_to_lambda (lookup v)
  | Mswitch (scr, cases) ->
    let scr = to_lambda scr in
    let rec flatten acc = function
      | ([], _) :: _ -> assert false
      | ([sel], e) :: rest -> flatten ((sel, to_lambda e) :: acc) rest
      | (sels, e) :: rest ->
        let i = next_raise_count () in
        let cases = List.map (fun s -> s, Lstaticraise(i, [])) sels in
        Lstaticcatch (flatten (cases @ acc) rest, (i, []), to_lambda e)
      | [] ->
        let rec partition (ints, tags, deftag) = function
          ...
          | (`Intrange _, _) as c :: cases -> partition (c :: ints, tags, deftag) cases
        in
        let (intcases, tagcases, deftag) = partition ([],[],None) (List.rev acc) in
          lbind "switch" scr begin fun scr ->
            ...
            match intswitch, tagswitch with
              ...
              | Some eint, Some etag -> Lifthenelse (lprim Pisint [scr], eint, etag)
          end
        in
        begin match cases with
          ...
          | [_, enonzero; [`Intrange (0, 0)], ezero] ->
              ...
              Lifthenelse(scr, to_lambda enonzero, to_lambda ezero)
          | cases -> flatten [] cases
        end
  | Mnumop1 (op, ty, e) ->
    let e = to_lambda e in
    ...
    let code = match op, ty with
      ...
      | `Not, `Bigint -> builtin ["Z"; "lognot"] [e] 
      | `Not, `Float64 -> assert false in
    code
  | Mnumop2 (op, ((`Int|`Int32|`Int64) as ty), e1, e2) ->
    let e1 = to_lambda e1 in
    let e2 = to_lambda e2 in
  ...
  | Mnumop2 (op, `Float64, e1, e2) ->
    ...
    begin match op with
      ...
      | `Mod -> builtin ["SomStdlib"; "mod_float"] [e1; e2]
  ...
  | Mconvert (src, dst, e) ->
    ...
    begin match src, dst with
      ...
      | `Bigint, ((`Int|`Int32|`Int64) as dst) ->
        ...
        let truncated =
          lbind "range"
            (builtin ["Z"; "of_string"] [Lconst (Const_immstring (Z.to_string range))])
            (fun range ->
              lbind "masked"
                (builtin ["Z"; "logand"] [e;
                    builtin ["Z"; "sub"] [range;
                ...
                (fun masked ->
                  Lifthenelse (builtin ["Z"; "testbit"] 
                                [masked; Lconst (Const_base (Const_int (width - 1)))],
                              builtin ["Z"; "sub"] [masked; range],
                              masked))) in
      ...
      | ((`Int|`Int32|`Int64) as src), `Bigint ->
        ...
        in
        builtin ["Z"; fn] [e]
      | `Int, `Int32 -> lprim (Pbintofint Pint32) [e]
      | `Int, `Int64 -> lprim (Pbintofint Pint64) [e]
      | `Int32, `Int -> lprim (Pintofbint Pint32) [e]
      | `Int64, `Int -> lprim (Pintofbint Pint64) [e]
      | `Int32, `Int64 -> lprim (Pcvtbint(Pint32, Pint64)) [e]
      | `Int64, `Int32 -> lprim (Pcvtbint(Pint64, Pint32)) [e]
      | `Int, `Float64 -> lprim Pfloatofint [e]
      | `Int32, `Float64 -> builtin ["Int32"; "to_float"] [e]
      | `Int64, `Float64 -> builtin ["Int64"; "to_float"] [e]
      | `Bigint, `Float64 -> builtin ["Z"; "to_float"] [e]
      (* FIXME: error handling on overflow *)
      | `Float64, `Int -> lprim Pintoffloat [e]
      | `Float64, `Int32 -> builtin ["Int32"; "of_float"] [e]
      | `Float64, `Int64 -> builtin ["Int64"; "of_float"] [e]
      | `Float64, `Bigint -> builtin ["Z"; "of_float"] [e]
    end
  | Mvecnew (`Array, len, def) ->
    builtin ["Array"; "make"] [to_lambda len; to_lambda def]
  | Mvecnew (`Bytevec, len, def) ->
    builtin ["String"; "make"] [to_lambda len; to_lambda def]
  | Mvecget (ty, vec, idx) ->
    let prim = match ty with
    ...
    in
    lprim prim [to_lambda vec; to_lambda idx]
  | Mvecset (ty, vec, idx, v) ->
    let prim = match ty with
    ...
    in
    lprim prim [to_lambda vec; to_lambda idx; to_lambda v]
  | Mveclen (ty, vec) ->
    let prim = match ty with
      | `Array -> Parraylength Paddrarray
      | `Bytevec -> Pbyteslength
    (*| `Floatvec -> Parraylength Pfloatarray *)
    in
    lprim prim [to_lambda vec]
  | Mblock (tag, vals) ->
    lprim (Pmakeblock (tag, Immutable, None)) (List.map to_lambda vals)
  | Mfield (idx, e) -> lprim (Pfield(idx)) [to_lambda e]
  | Mlazy e ->
    let fn = lfunction [fresh "param"] (to_lambda e) in
    lprim (Pmakeblock (Config.lazy_tag, Mutable, None)) [fn]
  | Mforce e -> Matching.inline_lazy_force (to_lambda e) loc_none

and bindings_to_lambda bindings body =
  List.fold_right begin fun b rest -> match b with
    | `Unnamed e ->
      Lsequence (to_lambda e, rest)
    | `Named (n, e) ->
      Llet (Strict, Pgenval, n, to_lambda e, rest)
    | `Recursive bs ->
      Lletrec (List.map (fun (n, e) -> (n, to_lambda e)) bs, rest)
    | `External (n, nn) ->
      add_external n nn;
      rest
  end bindings body
...
let setup_options () =
  ...
  Clflags.(
    (* TODO: implement -O command-line flag *)
...
let module_to_lambda ~module_name:_ ~module_id (Mmod (bindings, exports)) =
  setup_options ();
  ...
  env := Compmisc.initial_env ();
  let module_size, code =
    let bindings = reorder_bindings bindings in
    let exports = List.map (fun e -> snd (reorder e)) exports in
    if Config.flambda then
      List.length exports,
      bindings_to_lambda bindings
        (lprim (Pmakeblock (0, Immutable, None)) (List.map to_lambda exports))
    else begin
      let loc = loc_none (* TODO: actually have locations *) in
      ...
      let transl_exports subst =
        let exps = List.mapi (fun i e -> mod_store i (Subst.apply subst (to_lambda e))) exports in
      ...
      let rec transl_toplevel_bindings pos subst = function
        | `Unnamed e :: rest ->
          Lsequence (Subst.apply subst (to_lambda e),
            transl_toplevel_bindings pos subst rest)
        | `Named (n, e) :: rest ->
          let lam = Llet (
            Strict, Pgenval, n,
            Subst.apply subst (to_lambda e), mod_store pos (Lvar n))
          in
          Lsequence (lam,
        ...
        | `Recursive bs :: rest ->
          ...
          let stores = List.fold_right
            (fun x xs -> Lsequence (x, xs))
            stores (Lconst Lambda.const_unit)
          in
          let lam =
            Lletrec (bs |> List.map
              (fun (n, e) -> (n, Subst.apply subst (to_lambda e))),
              stores)
          in
...
let lambda_to_cmx ~filename ~prefixname ~module_name ~module_id lmod =
...
let compile_module ~filename modl =
  ...
  modl
  |> module_to_lambda ~module_name ~module_id
  |> lambda_to_cmx ~filename ~prefixname ~module_name ~module_id
