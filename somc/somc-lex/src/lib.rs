...
impl Lexer {
	fn identifier(&mut self) -> Token {
		...
		if ident == "true" { self.make_token(True) }
		else if ident == "false" { self.make_token(False) }
		// TODO: this below here thing:
		// else {
		...
		// 	}
		// }
		else { self.make_token(Identifier) }
	}
...
// public stuff
impl Lexer {
	pub fn new(filename: SString, source: *const Source) -> Self {
		Lexer{
			source,
			start_offset: 0,
			current_offset: 0,
